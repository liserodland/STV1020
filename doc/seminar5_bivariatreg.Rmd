---
title: 'Seminar 5: univariat regresjon'
author: "Lise Rødland"
date: "11 2 2021"
...
kjøre en univariat regresjon der vi ser på effekten av økonomisk vekst (`growth`)
på andel stemmer den sittende kandidaten får (`inc_vote`). Det første vi skal 
...
Når du lager en faktor-variabel så setter tar R utgangspunkt i alfabetet når 
...
til observasjonene vi plottet tidligere.
```{r}
ggplot(data = FairFPSR3) +
...
For å kjøre en univariat lineær regresjon i R så bruker vi funksjonen `lm()`. 
...
## Oppgaver
1. Last inn datasettet ess_no.  
2. Finn navn på variablene. 
3. Google noe! 
3. Plot 
2. Kjør en regresjonsmodell med hvor lykkelig en person er (happy) som avhengig 
variabel og i hvilken grad en opplever å ha innflytelse på det myndighetene gjør 
(influence) som uavhengig variabel. 
4. Tolk koeffisienten til influence. 
3. Print resultatet av regresjonen i en tabell ved hjelp av `stargazer()`. Lagre 
tabellen lokalt på pc-en din og åpne den i f.eks. word eller en nettleser. 
5. Lag et plott med de predikerte y-verdiene på y-aksen og verdiene av x på 
x-aksen. Lagre plottet lokalt på pc-en din.
